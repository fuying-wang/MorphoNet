...
'''
CUDA_VISIBLE_DEVICES=5 python create_connected_graph_survival.py --dataset TCGA_KIRC \
    --task survival \
...
CUDA_VISIBLE_DEVICES=5 python create_connected_graph_survival.py --dataset TCGA_LUAD \
    --task survival \
    --dataset_csv_dir /home/fywang/Documents/SPANTHER/src/dataset_csv \
    --agg_type ot \
    --skip_existed \
...
CUDA_VISIBLE_DEVICES=5 python create_connected_graph_survival.py --dataset TCGA_BRCA \
    --task survival \
    --dataset_csv_dir /home/fywang/Documents/SPANTHER/src/dataset_csv \
    --agg_type panther \
    --skip_existed \
...
def create_region_representations(x, attention, mode):
...
def main(attention, save_dir, save_h5_dir):
    ...
    for row in tqdm(df.itertuples(), total=len(df)):
        ...
        if args.skip_existed and os.path.exists(save_path) and os.path.exists(save_h5_path):
            continue
        ...
        for label_value in unique_labels:
            ...
            for region in regions:
                ...
                if args.agg_type in ["ot", "panther"]:
                    agg_center_embeddings = create_region_representations(region_embeddings, attention, mode=args.agg_type)
...
if __name__ == '__main__':
    for k in range(5):
        save_dir = os.path.join("/data1/r20user2/SPANTHER", f"{args.dataset}_overall_survival_k={k}",
                                f"extracted_mag20x_patch256/uni_pt_patch_features/superpatch_connected_{args.agg_type}_C{args.n_proto}_graph")
        save_h5_dir = os.path.join("/data1/r20user2/SPANTHER", f"{args.dataset}_overall_survival_k={k}",
                                f"extracted_mag20x_patch256/uni_pt_patch_features/superpatch_connected_{args.agg_type}_C{args.n_proto}") 
        os.makedirs(save_dir, exist_ok=True)
        os.makedirs(save_h5_dir, exist_ok=True)

        # check this for survival tasks
        proto_path = j_(args.split_dir, args.task, f"{args.dataset}_overall_survival_k={k}", 
                        f"prototypes/prototypes_c{args.n_proto}_uni_pt_patch_features_faiss_num_1.0e+05.pkl")
        ...
        main(attention, save_dir, save_h5_dir)