...
for dataroot_path in "${dataroots[@]}"; do
  ...
  # feat_dir=${dataroot_path}/extracted_mag${mag}_patch${patch_size}/${feat}/superpatch_ot_graph
  feat_dir=${dataroot_path}/extracted_mag${mag}_patch${patch_size}/${feat}/superpatch_leiden_mean_graph
#   feat_dir=/data1/r20user2/SPANTHER/${split_dir:9}/extracted_mag${mag}_patch${patch_size}/${feat}/superpatch_kmeans_mean_graph
  # echo "Using features from: $feat_dir"
  if ! test -d $feat_dir
  then
    continue
  fi
...
# all_feat_dirs=/data1/r20user2/SPANTHER/${split_dir:9}/extracted_mag${mag}_patch${patch_size}/${feat}/superpatch_connected_ot_C64_graph
# echo $all_feat_dirs
