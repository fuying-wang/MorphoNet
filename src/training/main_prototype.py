...
def main(args):
    ...
    save_fpath = j_(args.split_dir,
                    ...
                    f"prototypes_c{args.n_proto}_{args.data_source[0].split('/')[-2]}_{args.data_source[0].split('/')[-1]}_{args.mode}_num_{args.n_proto_patches:.1e}.pkl")
    save_pkl(save_fpath, {'prototypes': weights})

'''
CUDA_VISIBLE_DEVICES=0 python -m training.main_prototype \
--mode faiss \
--data_source /data1/r20user2/wsi_data/TCGA_BRCA/extracted_mag20x_patch256/uni_pt_patch_features \
--split_dir SPLIT_DIR \
--split_names train \
--in_dim 1024 \
