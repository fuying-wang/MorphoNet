...
GRAPH_MODELS = ['GNN', 'GraphTransformer']
...
def build_datasets(csv_splits, model_type, batch_size=1, num_workers=2, train_kwargs={}, val_kwargs={}):
    ...
    for k in csv_splits.keys(): # ['train', 'val', 'test']
        ...
        if model_type in GRAPH_MODELS:
            dataloader = GraphDataLoader(dataset, batch_size=1, num_workers=num_workers)
        else:
...
def main(args):
    ...
    train_kwargs = dict(data_source=args.data_source,
                        ...
                        shuffle=True,
                        load_cluster_feats=args.model_type == 'DeepAttnMISL',
                        )
...
### model args ###
parser.add_argument('--model_type', type=str, choices=['SPANTHER', 'GNN', 'DeepAttnMISL',
                                                       'H2T', 'ABMIL', 'TransMIL', 
                                                       'DSMIL', 'OT', 'PANTHER', 'ProtoCount', 
                                                       'DeepAttnMIL', 'ILRA', 'DeepSets',
                                                       'GraphTransformer'],
                    default='ABMIL',
                    help='type of model')
...
if __name__ == "__main__":
    ...
    if args.exp_code is None:
        if args.model_config in ['PANTHER_default', 'SPANTHER_default']:
