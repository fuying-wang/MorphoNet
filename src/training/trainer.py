...
PROTO_MODELS = ['SPANTHER', 'PANTHER', 'OT', 'H2T', 'ProtoCount', 'DeepSets']
...
def train(datasets, args, mode='classification'):
    ...
    assert args.es_metric in ["loss", "kappa"]
    ...
    for epoch in range(args.max_epochs):
        ...
        if mode == 'classification':
            train_results = train_loop_classification(model, datasets['train'], optimizer, lr_scheduler, loss_fn,
                                                      ...
                                                      accum_steps=args.accum_steps, model_type=args.model_type)
        elif mode == 'survival':
            train_results = train_loop_survival(model, datasets['train'], optimizer, lr_scheduler, loss_fn,
                                                in_dropout=args.in_dropout, print_every=args.print_every,
                                                accum_steps=args.accum_steps, model_type=args.model_type)
        elif mode == "sae":
            train_results = train_loop_sae(model, datasets['train'], optimizer, lr_scheduler, loss_fn,
                                           in_dropout=args.in_dropout, print_every=args.print_every,
        ...
        if 'val' in datasets.keys():
            ...
            if mode == 'classification':
                val_results, _ = validate_classification(model, datasets['val'], loss_fn,
                                                         print_every=args.print_every, verbose=True, model_type=args.model_type)
            elif mode == 'survival':
                val_results, _ = validate_survival(model, datasets['val'], loss_fn,
                                                   print_every=args.print_every, verbose=True, model_type=args.model_type)
            elif mode == "sae":
                val_results, _ = validate_loop_sae(model, datasets['val'], loss_fn,
            ...
            if early_stopper is not None:
                ...
                elif args.es_metric == "kappa":
                    score = val_results['kappa']
                else:
    ...
    for k, loader in datasets.items():
        ...
        if mode == 'classification':
            results[k], dumps[k] = validate_classification(model, loader, loss_fn, print_every=args.print_every,
                                                           dump_results=True, verbose=False, model_type=args.model_type)
        elif mode == 'survival':
            results[k], dumps[k] = validate_survival(model, loader, loss_fn, print_every=args.print_every,
                                                     dump_results=True, verbose=False, model_type=args.model_type)
        elif mode == "sae":
            results[k], dumps[k] = validate_loop_sae(model, loader, loss_fn, print_every=args.print_every,
                                                     dump_results=True, verbose=True)
        else:
            raise NotImplementedError("Unknown mode")
...
def train_loop_sae(model, loader, optimizer, lr_scheduler, loss_fn=None, in_dropout=0.0,
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        elif isinstance(batch["img"], Batch):
            data = batch['img'].to(device)
        elif isinstance(batch["img"], list):
            data = [x.to(device) for x in batch['img']]
        else:
        ...
        if in_dropout:
            if isinstance(data, list):
                data = [F.dropout(x, p=in_dropout) for x in data]
        ...
        if isinstance(data, torch.Tensor):
            batch_size = len(data)
            bag_size = data.size(1)
        elif isinstance(data, Batch):
            batch_size = 1
            bag_size = data.x.shape[0]
        elif isinstance(data, list):
            batch_size = len(data[0])
            # This is not correct for DeepAttnMISL
            bag_size = data[0].size(2)
...
def validate_loop_sae(model, loader, loss_fn=None, print_every=50, dump_results=False, verbose=1):
    model.eval()
    meters = {'bag_size': AverageMeter()}
    bag_size_meter = meters['bag_size']
...
def train_loop_survival(model, loader, optimizer, lr_scheduler, loss_fn=None, in_dropout=0.0, print_every=50,
                        accum_steps=32, model_type=None):
    model.train()
    meters = {'bag_size': AverageMeter()}
    bag_size_meter = meters['bag_size']
