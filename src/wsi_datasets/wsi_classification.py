...
class WSIClassificationDataset(Dataset):
    ...
    def __init__(self,
        ...
        for src in data_source:
            ...
            elif base_name in ['TEA_graph_25', 'superpatch_graph', 'WSI_graph', 'superpatch_connected_ot_graph',
                               'superpatch_connected_graph', 'superpatch_ot_graph', 'superpatch_connected_panther_graph',
                               'superpatch_connected_panther_C64_graph', 'superpatch_kmeans_mean_graph',
                               'superpatch_leiden_mean_graph'
                               ]:
                self.use_graph = True
            else:
                raise ValueError(f'Unknown data source: {base_name}')
        ...
        self.cluster_feats_path = os.path.join(self.data_source[0], "../cluster_feats")
    ...
    def __getitem__(self, idx):
        ...
        if self.use_graph:
            ...
            if self.load_cluster_feats:
                ...
                cluster_feats_path = os.path.join(self.cluster_feats_path, f'{slide_id}_faiss_num10.pt')
                # assert os.path.exists(cluster_feats_path), f'Cluster features not found at {cluster_feats_path}'
