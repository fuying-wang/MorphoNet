...
class WSIProtoDataset(Dataset):
    ...
    def __init__(self,
        ...
        for src in data_source:
            assert os.path.basename(src) in ['h5_files', 'pt_files', 'superpatch_connected', 'superpatch_mean']
            self.use_h5 = True if os.path.basename(src) in ['h5_files', 'superpatch_connected', 'superpatch_mean'] else False
    ...
    def __getitem__(self, idx):
        ...
        out = {'slide_id': self.get_sample_id(idx),
               'img': all_features,
               'coords': all_coords}
